...
var FACTORY_ADDRESS = process.env.REACT_APP_CHAIN_ID === '1001' ? '0xA0acE51272FD35A9AFA42C2694Ee27A390831Ee9' : '0xBAE324Fb546240E6Cd5Dbf423d6b2c83b4d8CC58';
var INIT_CODE_HASH = process.env.REACT_APP_CHAIN_ID === '1001' ? '0xe26f1edeb3c91a91d1eabfa8c3a718cd73b3b1abe7d4228919cb302be3985e52' : '0x4f1fb03b3f72f7040dbf0917b1c344254cb0c4bb4a51c3247dba5503a7c311b3';
...
Currency.KLAY = /*#__PURE__*/new Currency(18, 'KLAY', 'Klaytn');
var KLAY = Currency.KLAY;
...
var WKLAY = (_WKLAY = {}, _WKLAY[ChainId.CYPRESS] = /*#__PURE__*/new Token(ChainId.CYPRESS, '0xecDC29C1A9C286C771686301554C219D4dDaA93e', 18, 'WKLAY', 'Wrapped KLAY'), _WKLAY[ChainId.BAOBAB] = /*#__PURE__*/new Token(ChainId.BAOBAB, '0x7D6a947ee257188298E34e0E9F619b5Dc4b24ba6', 18, 'WKLAY', 'Wrapped KLAY'), _WKLAY);
...
var CurrencyAmount = /*#__PURE__*/function (_Fraction) {
  ...
  /**
   * Helper that calls the constructor with the KLAY currency
   * @param amount klay amount in wei
   */


  CurrencyAmount.klay = function klay(amount) {
    return new CurrencyAmount(KLAY, amount);
  };
...
var Price = /*#__PURE__*/function (_Fraction) {
  ...
  _proto.quote = function quote(currencyAmount) {
    ...
    return CurrencyAmount.klay(_Fraction.prototype.multiply.call(this, currencyAmount.raw).quotient);
  };
...
var Route = /*#__PURE__*/function () {
  function Route(pairs, input, output) {
    ...
    !(input instanceof Token && pairs[0].involvesToken(input) || input === KLAY && pairs[0].involvesToken(WKLAY[pairs[0].chainId])) ? process.env.NODE_ENV !== "production" ? invariant(false, 'INPUT') : invariant(false) : void 0;
    !(typeof output === 'undefined' || output instanceof Token && pairs[pairs.length - 1].involvesToken(output) || output === KLAY && pairs[pairs.length - 1].involvesToken(WKLAY[pairs[0].chainId])) ? process.env.NODE_ENV !== "production" ? invariant(false, 'OUTPUT') : invariant(false) : void 0;
    var path = [input instanceof Token ? input : WKLAY[pairs[0].chainId]];
 ...
 * In other words, if the currency is KLAY, returns the WKLAY token amount for the given chain. Otherwise, returns
 * the input currency amount.
 */

function wrappedAmount(currencyAmount, chainId) {
  if (currencyAmount instanceof TokenAmount) return currencyAmount;
  if (currencyAmount.currency === KLAY) return new TokenAmount(WKLAY[chainId], currencyAmount.raw);
   process.env.NODE_ENV !== "production" ? invariant(false, 'CURRENCY') : invariant(false) ;
}

function wrappedCurrency(currency, chainId) {
  if (currency instanceof Token) return currency;
  if (currency === KLAY) return WKLAY[chainId];
   process.env.NODE_ENV !== "production" ? invariant(false, 'CURRENCY') : invariant(false) ;
}
/**
...
var Trade = /*#__PURE__*/function () {
  function Trade(route, amount, tradeType) {
    ...
    this.inputAmount = tradeType === TradeType.EXACT_INPUT ? amount : route.input === KLAY ? CurrencyAmount.klay(amounts[0].raw) : amounts[0];
    this.outputAmount = tradeType === TradeType.EXACT_OUTPUT ? amount : route.output === KLAY ? CurrencyAmount.klay(amounts[amounts.length - 1].raw) : amounts[amounts.length - 1];
  ...
  _proto.minimumAmountOut = function minimumAmountOut(slippageTolerance) {
    ...
    if (this.tradeType === TradeType.EXACT_OUTPUT) {
      ...
      return this.outputAmount instanceof TokenAmount ? new TokenAmount(this.outputAmount.token, slippageAdjustedAmountOut) : CurrencyAmount.klay(slippageAdjustedAmountOut);
    }
  }
  /**
  ...
  _proto.maximumAmountIn = function maximumAmountIn(slippageTolerance) {
    ...
    if (this.tradeType === TradeType.EXACT_INPUT) {
      ...
      return this.inputAmount instanceof TokenAmount ? new TokenAmount(this.inputAmount.token, slippageAdjustedAmountIn) : CurrencyAmount.klay(slippageAdjustedAmountIn);
    }
  }
  /**
...
var Router = /*#__PURE__*/function () {
  ...
  Router.swapCallParameters = function swapCallParameters(trade, options) {
    var klayIn = trade.inputAmount.currency === KLAY;
    var klayOut = trade.outputAmount.currency === KLAY; // the router does not support both klay in and out

    !!(klayIn && klayOut) ? process.env.NODE_ENV !== "production" ? invariant(false, 'KLAY_IN_OUT') : invariant(false) : void 0;
    ...
    switch (trade.tradeType) {
      case TradeType.EXACT_INPUT:
        if (klayIn) {
          methodName = useFeeOnTransfer ? 'swapExactKLAYForTokensSupportingFeeOnTransferTokens' : 'swapExactKLAYForTokens'; // (uint amountOutMin, address[] calldata path, address to, uint deadline)

          args = [amountOut, path, to, deadline];
          value = amountIn;
        } else if (klayOut) {
          methodName = useFeeOnTransfer ? 'swapExactTokensForKLAYSupportingFeeOnTransferTokens' : 'swapExactTokensForKLAY'; // (uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline)
      ...
      case TradeType.EXACT_OUTPUT:
        ...
        if (klayIn) {
          methodName = 'swapKLAYForExactTokens'; // (uint amountOut, address[] calldata path, address to, uint deadline)

          args = [amountOut, path, to, deadline];
          value = amountIn;
        } else if (klayOut) {
          methodName = 'swapTokensForExactKLAY'; // (uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
...
var TOKEN_DECIMALS_CACHE = (_TOKEN_DECIMALS_CACHE = {}, _TOKEN_DECIMALS_CACHE[ChainId.CYPRESS] = {
...
export { ChainId, Currency, CurrencyAmount, FACTORY_ADDRESS, Fetcher, Fraction, INIT_CODE_HASH, InsufficientInputAmountError, InsufficientReservesError, KLAY, MINIMUM_LIQUIDITY, Pair, Percent, Price, Rounding, Route, Router, Token, TokenAmount, Trade, TradeType, WKLAY, currencyEquals, inputOutputComparator, tradeComparator };
