...
describe('Trade', () => {
  ...
  const token2 = new Token(ChainId.CYPRESS, '0x0000000000000000000000000000000000000003', 18, 't2')
  const token3 = new Token(ChainId.CYPRESS, '0x0000000000000000000000000000000000000004', 18, 't3')
  ...
  const pair_wklay_0 = new Pair(
    new TokenAmount(WKLAY[ChainId.CYPRESS], JSBI.BigInt(1000)),
  ...
  it('can be constructed with KLAY as input', () => {
    const trade = new Trade(
      new Route([pair_wklay_0], KLAY),
      CurrencyAmount.klay(JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
    )
    expect(trade.inputAmount.currency).toEqual(KLAY)
    expect(trade.outputAmount.currency).toEqual(token0)
  })
  it('can be constructed with KLAY as input for exact output', () => {
    const trade = new Trade(
      new Route([pair_wklay_0], KLAY, token0),
  ...
  it('can be constructed with KLAY as output', () => {
    const trade = new Trade(
      new Route([pair_wklay_0], token0, KLAY),
      CurrencyAmount.klay(JSBI.BigInt(100)),
      TradeType.EXACT_OUTPUT
    )
    expect(trade.inputAmount.currency).toEqual(token0)
    expect(trade.outputAmount.currency).toEqual(KLAY)
  })
  it('can be constructed with KLAY as output for exact input', () => {
    const trade = new Trade(
      new Route([pair_wklay_0], token0, KLAY),
      new TokenAmount(token0, JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for KLAY currency input', () => {
      const result = Trade.bestTradeExactIn(
        [pair_wklay_0, pair_0_1, pair_0_3, pair_1_3],
        CurrencyAmount.klay(JSBI.BigInt(100)),
        token3
      )
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(KLAY)
      expect(result[0].route.path).toEqual([WKLAY[ChainId.CYPRESS], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(KLAY)
      expect(result[1].route.path).toEqual([WKLAY[ChainId.CYPRESS], token0, token3])
      expect(result[1].outputAmount.currency).toEqual(token3)
    })
    it('works for KLAY currency output', () => {
      const result = Trade.bestTradeExactIn(
        [pair_wklay_0, pair_0_1, pair_0_3, pair_1_3],
      ...
      expect(result[0].route.path).toEqual([token3, token0, WKLAY[ChainId.CYPRESS]])
      expect(result[0].outputAmount.currency).toEqual(KLAY)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WKLAY[ChainId.CYPRESS]])
      expect(result[1].outputAmount.currency).toEqual(KLAY)
    })
  })
