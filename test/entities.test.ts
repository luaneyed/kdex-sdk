...
import { ChainId, WKLAY as _WKLAY, TradeType, Rounding, Token, TokenAmount, Pair, Route, Trade } from '../src'
...
const CHAIN_ID = ChainId.BAOBAB
const WKLAY = _WKLAY[ChainId.BAOBAB]
    ...
    describe(`decimals permutation: ${decimals}`, () => {
      ...
      it('Pair', () => {
        pairs = [
          ...
          new Pair(
            new TokenAmount(tokens[2], decimalize(1, tokens[2].decimals)),
            new TokenAmount(WKLAY, decimalize(1234, WKLAY.decimals))
          )
        ]
      })
      ...
      it('Route', () => {
        ...
        expect(route.path).toEqual(tokens.concat([WKLAY]))
        expect(route.input).toEqual(tokens[0])
        expect(route.output).toEqual(WKLAY)
      })
      ...
      describe('Trade', () => {
          ...
          route = new Route(
            [
              new Pair(
                ...
                new TokenAmount(WKLAY, decimalize(10, WKLAY.decimals))
              )
            ],
            tokens[1]
          )
          const inputAmount = new TokenAmount(tokens[1], decimalize(1, tokens[1].decimals))
          const expectedOutputAmount = new TokenAmount(WKLAY, '1662497915624478906')
        ...
        it('TradeType.EXACT_OUTPUT', () => {
          const outputAmount = new TokenAmount(WKLAY, '1662497915624478906')
      ...
      it('TokenAmount', () => {
        const amount = new TokenAmount(WKLAY, '1234567000000000000000')
